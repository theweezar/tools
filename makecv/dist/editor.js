      ...
      exports2["default"] = function(instance) {
        instance.registerHelper("log", function() {
          ...
          if (options.hash.level != null) {
            level2 = options.hash.level;
          } else if (options.data && options.data.level != null) {
            level2 = options.data.level;
          }
          args[0] = level2;
  ...
  var require_logger = __commonJS({
    "node_modules/handlebars/dist/cjs/handlebars/logger.js"(exports2, module2) {
      ...
      var logger = {
        ...
        lookupLevel: function lookupLevel(level2) {
          if (typeof level2 === "string") {
            var levelMap = _utils.indexOf(logger.methodMap, level2.toLowerCase());
            if (levelMap >= 0) {
              level2 = levelMap;
            } else {
              level2 = parseInt(level2, 10);
            }
          }
          return level2;
        },
        // Can be overridden in the host environment
        log: function log(level2) {
          level2 = logger.lookupLevel(level2);
          if (typeof console !== "undefined" && logger.lookupLevel(logger.level) <= level2) {
            var method = logger.methodMap[level2];
            if (!console[method]) {
              method = "log";
            }
  ...
  EVENTS.forEach((eventName) => {
    document.addEventListener(eventName, function() {
      ...
      Array.from(document.querySelectorAll(".ql-container")).forEach((node) => {
        const quill2 = instances_default.get(node);
        if (quill2 && quill2.emitter) {
          quill2.emitter.handleDOM(...args);
        }
      });
    });
  });
...
${escapeText(this.code(index, length))}
  ...
  function deleteRange(_ref2) {
    ...
    const lines = quill2.getLines(range);
    let formats = {};
    if (lines.length > 1) {
    ...
    quill2.deleteText(range, Quill.sources.USER);
    if (Object.keys(formats).length > 0) {
      quill2.formatLine(range.index, 1, formats, Quill.sources.USER);
    }
    quill2.setSelection(range.index, Quill.sources.SILENT);
  }
  ...
  var Uploader = class extends module_default {
    constructor(quill2, options) {
      ...
      quill2.root.addEventListener("drop", (e) => {
        ...
        const normalized = native && quill2.selection.normalizeNative(native);
        if (normalized) {
          const range = quill2.selection.normalizedToRange(normalized);
  ...
  var Input = class extends module_default {
    constructor(quill2, options) {
      super(quill2, options);
      quill2.root.addEventListener("beforeinput", (event) => {
        this.handleBeforeInput(event);
      });
      if (!/Android/i.test(navigator.userAgent)) {
        quill2.on(Quill.events.COMPOSITION_BEFORE_START, () => {
...
${escapeText(this.code(index, length))}
  ...
  var Tooltip = class {
    constructor(quill2, boundsContainer) {
      this.quill = quill2;
